...
<div class="container">
  <el-card class="box-card" body-style="padding:0;" v-if="hasPermission('query')">
    <div slot="header" class="clearfix">
      ...
      <el-button type="success" class="shadow" @click="$router.push('articles/0')" v-if="hasPermission('create')">添加文章</el-button>
    </div>
    <el-table v-loading="loading" element-loading-text="拼命加载中" :data="list" border :height="containerH" row-class-name="simsir-table-row">
      ...
      <el-table-column label="操作" width="160" align="center">
        <template slot-scope="scope">
          <el-button @click="$router.push('articles/' + scope.row.id)" size="small" type="primary" v-if="hasPermission('update')">编辑</el-button>
